...
export default function ButtonShowCategory(props: IButtonShowCategoryProps) {
    ...
    return (
        <div className=''>
            ...
            <div
                className={`fixed top-0 right-0 z-50 shadow-md transition-all duration-500  ${!showListCategory ? "top-[-100%]" : "top-0"
                    }`}
            >
                <ListHomeCategory wrapperRefCategory={wrapperRefCategory} setOpenListCategory={setShowListCategory} />
                <div className={`min-h-screen bg-[#ffffffc3] ${showListCategory ? "block" : "hidden"}`}></div>
            </div>
        </div>
    );
}
