    ...
    <div className="flex hover:shadow-lg hover:shadow-gray-600 group  p-2 px-4 text-sm gap-3   rounded-md   bg-white ">
      ...
      <div className="flex flex-col gap-2 w-4/5">
        <Link href={`/truyen/${convertToSlug(book?.name)}-${book?._id}.html`}>
          ...
          <p
            onClick={() => handleUpdateView(book?._id)}
            className=" text-base font-medium max-w-[300px] md:w-[250px] overflow-hidden truncate hover:text-[#128c7e]"
          >
            {book?.name}
          </p>
        </Link>
        ...
        >
          {" "}
          <p className="hover:text-[#007bff] font-medium">
            {book.authors[0].name}
          </p>
        </Link>
        <ul className="flex items-center text-xs h-[25px] max-w-[150px] overflow-hidden  gap-1 flex-wrap ">
          {book.categories.map((category, index) => (
            ...
            >
              <li className="p-[2px] cursor-pointer px-1 hover:text-[#22C55E] border rounded-full border-[#22C55E]">
                {category.name}
              </li>
            </Link>
          ))}
