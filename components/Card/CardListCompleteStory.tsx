  ...
  return (
    ...
    >
      <Link href={`/truyen/${convertToSlug(book?.name)}-${book?._id}.html`}>
        ...
        >
          ...
          >
            ...
            <div
              style={{ transform: "perspective(500px) rotateY(-50deg)" }}
              className="absolute  flex items-center top-0 -z-10  p-1 w-[16px]   -left-4  h-full   justify-center bg-gradient-to-b from-[#eee] to-[#aaa] text-center text-xs "
            >
      ...
      <div className="flex flex-col text-xs gap-1 ">
        <p
          onClick={() => handleUpdateView(book?._id)}
          className="text-lg cursor-pointer overflow-hidden hover:text-[#128c7e] font-semibold w-[300px] md:w-[250px] truncate"
        >
          <Link href={`/truyen/${convertToSlug(book?.name)}-${book?._id}.html`}>
            {book?.name}
          </Link>
