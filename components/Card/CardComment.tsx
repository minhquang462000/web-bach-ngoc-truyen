        ...
        <div className='flex gap-3 w-full  text-sm '>
            ...
            <div className=' flex flex-col '>
                <div className='bg-[#eeeeee2c] max-w-[300px] md:max-w-full p-3 rounded-lg'>
                    ...
                    <p className={`text-wrap my-2   text-sm whitespace-pre-wrap overflow-hidden truncate ${showAllComment ? 'h-max' : 'max-h-[60px] md:line-clamp-3'}`}>
                        {content}
                    </p>
                    <button
                        onClick={() => setShowAllComment(!showAllComment)}
                        className={`font-bold pt-2 ${content.length > 200 ? 'block' : 'hidden'}`}>{showAllComment ? 'Ẩn bớt' : 'Xem thêm'}</button>
                </div>
                <ul className='flex  mt-1 items-center gap-1'>
                    <li className='flex  gap-1  items-center'><FaRegThumbsUp /> 1</li>
                    <li><LuDot /></li>
                    <li className='flex    items-center'><IoIosUndo size={17} /> Trả lời</li>
                    <li><LuDot /></li>
                    <li>7 tháng</li>
                </ul>
            </div>
