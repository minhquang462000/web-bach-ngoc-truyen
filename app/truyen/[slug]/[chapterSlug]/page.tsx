...
export default async function page({ params, searchParams }: PropParams) {
  const pageComment = Number((await searchParams)?.pageComment);
  return (
    <MainLayout>
        ...
        <div className="flex-col flex gap-4 p-2 lg:max-w-[1140px] m-auto">
          ...
          <div className=" md:w-4/5 m-auto w-full text-lg  flex flex-col gap-2">
            ...
            <ul className="flex justify-center text-sm m-auto md:w-[400px] w-full items-center font-medium text-[#128c7e] ">
              <li className="flex flex-col h-max py-3 justify-center w-[140px] gap-2 hover:shadow-md  cursor-pointer duration-500 transition-all hover:-translate-y-1 hover:shadow-[#327068] rounded-md items-center">
          ...
          <div className="text-white md:w-2/3 m-auto">
            <ListCommentDetailItem page={pageComment} />
          </div>
        </div>
      </main>
    </MainLayout>
