...
import { IAuthor, IBook, IFilter } from '@/interfaces';
...
export default async function page({ params }: { params: { slug: string } }) {
    ...
    const bookSameAuthor = await getListBooksNoTotal({ author: id, page: 0, limit: 12 } as IFilter)
    return (
        <MainLayout>
            <main className='lg:max-w-[1140px] w-full md:max-w-[705px] flex flex-col gap-3 m-auto'>
                <div className='w-full flex flex-col md:flex-row py-4 justify-between '>
                    <h2 className='text-center font-bold text-2xl w-max p-3'>{author?.name}</h2>
                    <ButtonShowCategory />
                </div>
